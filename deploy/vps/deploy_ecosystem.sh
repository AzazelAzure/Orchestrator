...
if [[ "$SKIP_HFM" -eq 0 ]]; then
  log "sync HFM proxy ecosystem files (ecosystem-hosts deploy is HFM-owned)"
  rsync_to "$HFM_ROOT/proxy/conf.d/ecosystem-hosts.conf.template" '~/finance_manager/proxy/conf.d/'
  rsync_to "$HFM_ROOT/scripts/ops/render_ecosystem_hosts.sh" '~/finance_manager/scripts/ops/'
  rsync_to "$HFM_ROOT/scripts/ops/attach_orchestrator_proxy_networks.sh" '~/finance_manager/scripts/ops/'
