...
check_presentation_color() {
  ...
  curl -fsS "http://127.0.0.1:${api_port}/health/" >/dev/null || fail "loopback API :${api_port}/health/"
  log "ok: api-$color loopback :${api_port}/health/"

  if podman container exists "$console_name" 2>/dev/null; then
    curl -fsS "http://127.0.0.1:${console_port}/" >/dev/null || fail "loopback console :${console_port}/"
    log "ok: console-$color loopback :${console_port}/"
  else
