...
- **Codex terminal identity fallback** — Codex 0.146.0 emits `thread_id` on
  ...
  `request_id` across the bounded stream and uses it only when a validated terminal
  event is present but lacks identity, so exit-0 token-bearing acceptance settles
  `complete` instead of `outcome_unknown`. Early identity without any terminal event
  remains `outcome_unknown`. Bounded redacted evidence no longer appends a duplicate
